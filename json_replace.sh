...
process_json_file() {
  ...
  if [[ "$jsonpath" == ..* ]]; then
    ...
    if [[ -n "$prefix" || -n "$suffix" ]]; then
      jq_filter="walk(if type == \"object\" and has(\"inner\") then .inner |= (if has(\"name\") then .name |= (\$prefix + . + \$suffix) else . end) else . end)"
...
usage() {
  ...
  echo "  Recursive with prefix/suffix: $0 -t target/sample.json -p '..inner.name' -x 'prefix_' -y '_suffix'"
  exit 1
}
